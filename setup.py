...
with open("app/__init__.py", "w") as f:
    f.write("""# ФинНавигатор - AI ассистент для личных финансов
""")
...
app = FastAPI(title="ФинНавигатор")
...
@app.get("/")
async def root():
    return {"message": "ФинНавигатор работает!", "status": "ok"}
...
with open("app/routers/profile.py", "w") as f:
...
with open("app/routers/goals.py", "w") as f:
    f.write("""from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
...
<body>
    <div class="container">
        ...
        <div class="status">
            <p>✅ Сервер работает!</p>
            <p>ФинНавигатор — ваш AI-ассистент для управления личными финансами.</p>
        </div>
        <button class="btn" onclick="checkHealth()">Проверить сервер</button>
    ...
    <script>
        async function checkHealth() {
            try {
                ...
                document.getElementById('result').innerHTML = '✅ Сервер отвечает: ' + JSON.stringify(data);
            } catch(e) {
                document.getElementById('result').innerHTML = '❌ Ошибка подключения: ' + e.message;
            }
        }
    </script>
</body>
...
print("")
print("Затем откройте в браузере: http://localhost:8000")